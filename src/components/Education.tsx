    ...
    >
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3">
          <div className="relative">
            ...
            <div className="space-y-5">
              {education.map((edu, i) => (
                <div key={edu.degree} className="reveal relative pl-14" data-reveal-delay={String(i * 90)}>
                  <span className="absolute left-0 top-1 flex h-9 w-9 items-center justify-center rounded-xl border border-sky-500/40 bg-slate-50 text-sky-600 transition-transform duration-300 hover:scale-110 dark:bg-slate-900 dark:text-accent-400">
                  ...
                  <div className="glass-card p-5">
                    ...
                    <div className="mt-3 inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-slate-100 px-3 py-1.5 dark:border-slate-700 dark:bg-slate-800/60">
        ...
        <div className="space-y-5 lg:col-span-2">
          <div className="glass-card reveal p-6" data-reveal-delay="120">
            <div className="mb-4 flex items-center gap-2.5">
              <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-sky-500/30 bg-sky-500/10 text-sky-600 dark:text-accent-400">
            ...
            <ul className="space-y-3">
              {achievements.awards.map((a) => (
                <li key={a} className="flex items-start gap-2.5 text-sm leading-relaxed text-slate-700 dark:text-slate-300">
                  <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-sky-500 dark:bg-accent-400" />
                  {a}
                </li>
              ))}
            </ul>
          ...
          <div className="glass-card reveal p-6" data-reveal-delay="200">
            <div className="mb-4 flex items-center gap-2.5">
              <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-cyan-500/30 bg-cyan-500/10 text-cyan-600 dark:border-cyan-glow/30 dark:bg-cyan-glow/10 dark:text-cyan-glow">
            ...
            <ul className="space-y-3">
              {achievements.extracurricular.map((e) => (
                <li key={e} className="flex items-start gap-2.5 text-sm leading-relaxed text-slate-700 dark:text-slate-300">
                  <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-cyan-500 dark:bg-cyan-glow" />
                  {e}
                </li>
              ))}
            </ul>
